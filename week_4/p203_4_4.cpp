...
void saveGame(int** map, int size, vector<Move>& moves, string startTime) {
    ...
    file << "마지막 저장 시간: " << getCurrentTime() << endl;
...
// 저장된 게임 불러오기 함수
bool loadGame(int**& map, int& size, vector<Move>& moves, string& startTime) {
    ...
    string line, word;
    getline(file, line); // 게임 시작 시간
    startTime = line.substr(10);
    getline(file, line); // 마지막 저장 시간
    getline(file, line); // 맵 크기
    ...
    // 움직임 기록 읽기
    getline(file, line); // 빈 줄
    getline(file, line); // "움직임 기록:"
    while (getline(file, line)) {
        Move move;
        sscanf(line.c_str(), "위치: (%d, %d), 방향: %c", &move.x, &move.y, &move.direction);
        moves.push_back(move);
    }

    file.close();
    cout << "저장된 게임을 불러왔습니다!" << endl;
    return true;
}
...
void playGame(bool isNewGame) {
    ...
    if (isNewGame) {
        ...
        // 맵 생성
        randMap(map, size);
        startTime = getCurrentTime(); // 게임 시작 시간 저장
    } else {
        if (!loadGame(map, size, moves, startTime)) return;
    }
